...
def convert_world_to_screen_position(screen:Surface, position: Vector2) -> tuple[float, float]:
    return (screen.get_width() / 2 + position.x / 1_000_000, screen.get_height() /2 - position.y / 1_000_000)

def convert_world_to_screen_length(length: int) -> float:
...
def draw_image_to_circle(screen: Surface, circle: Circle, image: Surface):
    size = (convert_world_to_screen_length(circle.radius * 2), convert_world_to_screen_length(circle.radius * 2))
